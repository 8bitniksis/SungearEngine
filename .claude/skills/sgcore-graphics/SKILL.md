...
Состояние 2026-08-22: **DX12 сравнялся с Vulkan** — смоук против GL46 даёт 46.166 % (было 54.303 %),
у Vulkan 46.171 %; друг от друга бэкенды расходятся на 0.370 % при строгом допуске (max diff 48).
Геометрические вложения 1/4/5/6 — 0.000 % против GL46, вложение 2 FX-буфера (окклюзия SSAO) — 39.122 %
**ровно как у Vulkan**, то есть остаток целиком общий для explicit-бэкендов (шумовая текстура SSAO,
см. «Мелкие грабли API»), своего у DX12 больше нет. `SGRHITest --gapi dx12` — PASS, gl46 и vulkan без
...
⚠️ Изредка процесс DX12 **зависает при выходе** после смоука — запускать под таймаутом.
...
- **`ITexture2D::create<T>(data, …)` не выставляет `m_dataType` и копирует `w*h*размер_internalFormat`
  ...
  **Остаток — расхождение самой математики прохода** (вложение 2 — 39.1 %, финальный кадр — 46.2 %).
  Проверено пробниками, что идентичны: всё ядро `SG_SSAO_samples[]` (сумма по всем 64 элементам +
  последний элемент), `SG_SSAO_samplesCount`, шум, G-буфер (0.000 %), камера. То есть входы сошлись, а
  результат — нет; искать надо в самом цикле окклюзии.
  ⚠️ **Метрика при этом ухудшилась**: 7.138 % → 46.171 %. Это не регресс качества, а обратное — раньше
  на Vulkan эффекта не было вовсе, и плоский результат случайно оказывался численно ближе к эталону.
