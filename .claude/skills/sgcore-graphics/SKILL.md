...
FX-фреймбуфера, RGBA32F): на Vulkan это шум. Предупреждений о раскладке изображений в логе нет,
то есть текстуры сэмплируются в правильном layout'е; разбирать надо сам композит
(`PostProcessPass::bindCommonUniforms` — юнит 6 сэмплирует буфер глубины, юнит 7 читает то самое
вложение 7, в которое проход рисует).
