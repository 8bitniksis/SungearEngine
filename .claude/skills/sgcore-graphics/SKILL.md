...
Состояние на 2026-08-18: **геометрический проход на Vulkan совпадает с GL46** — вложения 0, 1, 2, 4,
5, 6, 7 дают 0.000 %, вложение 3 (стохастическая прозрачность) — 0.108 % (сам дизеринг на границах).
Вложения 0 и 1 FX-фреймбуфера — тоже 0.000 %. Расходится только финальная композиция (вложение 7
FX-фреймбуфера, RGBA32F): на Vulkan это шум. Что уже установлено: предупреждений о раскладке
изображений нет; шум **порождается** композитом, а не остаётся от неинициализированной памяти
(очистка вложений при создании его не убрала). Причина сужена до гейта в
`layer_default_fx.glsl:116` — `texture(SGPP_LayersVolumes, uv).rgb == calculatePPLayerVolume(SGPP_CurrentLayerIndex).rgb`
проходит на GL и **не проходит на Vulkan**, а при непрохождении `fragColor` не присваивается вовсе
(тот же класс дефекта, что уже дважды находился). NaN ни с одной стороны нет; значения близки, но не
равны. Проверять дальше: чем именно отличается сэмпл `SGPP_LayersVolumes` и что лежит в
`SGPP_CurrentLayerIndex` на каждом бэкенде.
