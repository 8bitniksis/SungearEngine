...
Состояние на 2026-08-18: геометрический проход на Vulkan против GL46 — **1.582 % расхождения**, и это
ровно прозрачная сфера: она пишется во вложение 1, тогда как GL уводит её в стохастическую
прозрачность. Всё остальное (небо, пол, куб, бокс, непрозрачная сфера, тени, блик) совпадает.
`SGRHITest` — PASS на gl46 и vulkan; `SGSmokeTest --gapi gl46 --reference` — 0.000 %.
Дальше: стохастическая прозрачность и цепочка постобработки (FX).
