...
| 1.4b Компиляция в SPIR-V (glslang, target Vulkan 1.3) + рефлексия (SPIRV-Reflect) → `ShaderReflection` в шейдер-ассете; кеш по хешу исходник+дефайны; DXIL для DX12 через spirv-cross → HLSL → DXC | @Senior Graphics Engineer | L | ⬜ **ждёт добавления `glslang`, `spirv-reflect` (позже `spirv-cross`) в `vcpkg.json`** — требуется явное согласие |
...
| 1.6b **`GL46Renderer` сломан** (обнаружено смоук-тестом): на GL46 кадр чёрный — `GL_INVALID_ENUM` при каждой загрузке текстуры (`GL46Texture2D`), геометрические шейдеры batching/terrain не компилируются под `#version 460` («No input primitive type»). Решить: чинить GL46 или объявить `GL4Renderer` единственным GL-бэкендом и наращивать в нём 4.6-фичи (см. RHI_DESIGN, вопрос №5) | @Senior Graphics Engineer | M | ⬜ решение до задачи 1.5 |
