...
| 2.3 Ресурсы: RHI на Vulkan **сделан** (`RHI/`: `VulkanDevice`, `VulkanGPUBuffer` (VMA), `VulkanTexture`, `VulkanShaderProgram` (SPIR-V + layout'ы по рефлексии), `VulkanPipelineState` (варианты по форматам прохода), `VulkanDescriptorSet` (транзиентные наборы), `VulkanCommandList` (dynamic rendering, transfer-буфер для аплоадов в проходе)); фасады `VkTexture2D` и `VkFrameBuffer` (render pass из bind/unbind, readback) — **сделаны**. **Осталось**: фасад `VkShader` над `IShaderProgram` (UBO по рефлексии, как `GL46Shader`) + таблица юнитов → `IDescriptorSet`, `VkMeshData`/`VkVertexArray`/`VkVertexBuffer`/`VkIndexBuffer`/`VkUniformBuffer`/`VkCubemapTexture` (пока заглушки 2023 г.), `renderMeshData`/`renderArray` в `VkRenderer` по образцу GL46; depth-only view для сэмплирования depth-stencil; переименование `Vk*` → `Vulkan*` (текущие имена не конфликтуют с типами Vulkan, но путают) | @Senior Graphics Engineer | L | 🟡 в работе |
