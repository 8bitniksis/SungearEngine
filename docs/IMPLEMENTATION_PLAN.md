...
| 1.5c Фасад `IRenderer` поверх `IDevice` и миграция проходов PBRRP на `ICommandList` (порядок из RHI_DESIGN: screen quad → PBRRP geometry → CSM → PostProcess → остальные) под смоук-контролем; `IShader` → `IShaderProgram` через вулканизатор; push constants на GL через мини-UBO; удаление фасада | @Senior Graphics Engineer | L | ⬜ следующий шаг |
