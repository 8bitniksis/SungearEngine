...
| 2.3 Ресурсы: буферы/текстуры через VMA, реализация RHI-интерфейсов (замена скелета `Graphics/API/Vulkan/` 2023 г.); **фасад legacy `IShader`/`ITexture2D`/`IFrameBuffer` над Vulkan-устройством по образцу GL46** (UBO по рефлексии, таблица юнитов → дескрипторные наборы, render pass из bind/unbind) — чтобы проходы заработали без правки | @Senior Graphics Engineer | L | ⬜ |
