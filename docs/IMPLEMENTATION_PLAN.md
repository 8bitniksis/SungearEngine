...
| 6 | Лицензия и «сторона Unity»: движок под GPL-3.0 — игры, линкующиеся с ядром (в т.ч. через C#-скриптинг), обязаны быть GPL. Это осознанно, или для скриптингового API нужна отдельная лицензия (LGPL/исключение для линковки)? | Этап 6, привлечение внешних разработчиков игр | Решить **до** публикации скриптингового API: менять лицензию после появления пользователей больно |
