...
**Осталось**: (1) прозрачная сфера пишется во вложение 1, тогда как GL уводит её в стохастическую прозрачность — это ровно оставшиеся 1.582 %; (2) постобработка (цепочка FX-проходов); (3) `renderArray` выдаёт `vkCmdDrawIndexed(0, 1)` при нулевом числе индексов — draw надо пропускать; (4) имена attachment'ов в отладке неуникальны между фреймбуферами (в RenderDoc не различить); (5) соответствие color-attachment'ов выходам шейдера (18 предупреждений валидации) | @Senior Graphics Engineer | L | 🟡 в работе |
