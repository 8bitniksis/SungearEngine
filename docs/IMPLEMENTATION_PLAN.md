...
**Осталось**: (1) финальная композиция (вложение 7 FX-фреймбуфера, RGBA32F) на Vulkan — по-прежнему шум. Установлено: шум **порождается** композитом (очистка вложений его не убрала), предупреждений о раскладке изображений нет, все входы совпадают с GL46. Причина сужена до гейта `layer_default_fx.glsl:116` — `texture(SGPP_LayersVolumes, uv).rgb == calculatePPLayerVolume(SGPP_CurrentLayerIndex).rgb` проходит на GL и не проходит на Vulkan, а при непрохождении `fragColor` не присваивается вовсе (тот же класс дефекта, что уже дважды находился); NaN ни с одной стороны нет, значения близки, но не равны; (2) `IShader::bindTextures` связывает текстуру с юнитом 0, а сэмплеру сообщает `samplersOffset` — расходящаяся пара, на юнит-модели Vulkan это даёт не ту текстуру; (3) `renderArray` выдаёт `vkCmdDrawIndexed(0, 1)` при нулевом числе индексов — draw надо пропускать; (4) прогон `SGSmokeTest --gapi vulkan` иногда зависает при завершении (процесс не убивается, держит exe); (5) имена attachment'ов в отладке неуникальны между фреймбуферами; (6) соответствие color-attachment'ов выходам шейдера (18 предупреждений валидации) | @Senior Graphics Engineer | L | 🟡 в работе |
