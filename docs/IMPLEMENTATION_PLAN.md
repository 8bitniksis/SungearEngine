...
**Осталось**: (1) атмосфера на Vulkan не рисуется (белый фон вместо неба) — основная часть оставшихся 47 % расхождения вложения 1 с GL46; (2) прозрачная сфера пишется во вложение 1, тогда как GL уводит её в стохастическую прозрачность; (3) постобработка (цепочка FX-проходов); (4) `renderArray` выдаёт `vkCmdDrawIndexed(0, 1)` при нулевом числе индексов — draw надо пропускать; (5) имена attachment'ов в отладке неуникальны между фреймбуферами (в RenderDoc не различить); (6) соответствие color-attachment'ов выходам шейдера (18 предупреждений валидации) | @Senior Graphics Engineer | L | 🟡 в работе |
