...
| 5 | Нужен ли GL 4.x-fallback ниже 4.6 (текущий `GL4`-бэкенд)? | Оставить один GL46 на ПК: `ARB_gl_spirv`, DSA и compute есть только в 4.5+; железо без GL 4.6 (до ~2012 г.) вне целевой аудитории. `GL4` удаляется при миграции 1.5 | задача 1.5 |
