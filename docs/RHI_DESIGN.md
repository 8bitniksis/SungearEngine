...
- Один интерфейс, под которым честно живут Vulkan, DX12 и текущий GL46.
...
├── GL/                             # GL46 мигрирует на новый RHI (задача 1.5)
...
| 3 | Минимальные требования: Vulkan 1.3, DX12 FL 12.0, GL 4.6 — отсекает старое железо | Принять; GL46-бэкенд остаётся fallback'ом (вопрос №2 плана) | утверждение этого документа |
