...
- **Между транслятором и glslang стоит проход «вулканизации»**
  ...
  собирает свободные юниформы в один `layout(std140, set, binding) uniform
  SGLegacyUniforms {…}` — члены блока без имени экземпляра видны под старыми
  именами, поэтому **тела шейдеров не меняются**; назначает `set/binding`
  сэмплерам/UBO/TBO (явные сохраняются), переносит `#if`-условия объявлений
  на члены блока, заменяет `gl_FragColor`/`gl_VertexID`. Обрабатывается вся
  программа сразу — биндинги едины во всех стадиях. Это и есть «legacy-UBO»
  фасада: реальный источник данных для `IShader::useMatrix("имя", …)` на
  Vulkan/DX12; раскладка членов берётся из рефлексии.
...
| 5 | Какой GL-бэкенд становится постоянным fallback'ом: `GL46Renderer` или `GL4Renderer`? | ⚠️ Уточнено 2026-08-17: смоук-тест показал, что **`GL46Renderer` сломан** (чёрный кадр: `GL_INVALID_ENUM` в `GL46Texture2D`, геометрические шейдеры не компилируются под 460), рабочий сегодня — `GL4Renderer` (он же тянет `#version 400`-путь). Предложение: при миграции 1.5 делать **один** GL-бэкенд на базе рабочего `GL4Renderer`, поднимая контекст до 4.6 и добавляя DSA/`ARB_gl_spirv` пошагово под смоук-контролем; `GL46Renderer` в текущем виде удалить, а не чинить параллельно | решение до задачи 1.5 (задача 1.6b плана) |
